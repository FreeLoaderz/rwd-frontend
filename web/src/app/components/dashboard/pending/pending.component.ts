...
export class PendingComponent extends NotificationComponent implements OnInit, OnDestroy {
    public walletSubscription: Subscription;
    public walletLoaded: boolean = false;
    public pendingCols: any[] = [];
    public pendingReward: Array<HistoricalClaim> = [];
    ...
    constructor(public router: Router, public titleService: Title, public restService: RestService,
        ...
        this.exportFileName = "Pending_SmartClaimz_".concat(dateString);
    }
    ...
    public ngOnInit() {
        this.walletSubscription = this.walletObserverService.loaded$.subscribe(
            loaded => {
                ...
                if (loaded === true) {
                    this.restService.getPendingReward()
                        .then(res => this.processPending(res))
    ...
    public processPending(data: any) {
        if (data != null) {
            const tempPendingRewardArray: Array<HistoricalClaim> = [];
            for (let i = 0; i < data.length; ++i) {
                const historicalClaim = new HistoricalClaim(data[i]);
                tempPendingRewardArray.push(historicalClaim);
            }
            this.pendingReward = [...tempPendingRewardArray];
        }
    }
}